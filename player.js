...
function drawPitcher(){
    ...
    for(let i = 0; i < playerList.pitcher.length; i++){
        ...
        playerLi.onclick = function(){
            openDetail(JSON.stringify(playerList.pitcher[i]));
        };
...
function drawCatcher(){
    ...
    for(let i = 0; i < playerList.catcher.length; i++){
        ...
        playerLi.className = "player";
        playerLi.onclick = function(){
            openDetail(JSON.stringify(playerList.catcher[i]));
        };
...
function drawInfielder(){
    ...
    for(let i = 0; i < playerList.infielder.length; i++){
        ...
        playerLi.className = "player";
        playerLi.onclick = function(){
            openDetail(JSON.stringify(playerList.infielder[i]));
        };
...
function drawOutfielder(){
    ...
    for(let i = 0; i < playerList.outfielder.length; i++){
        ...
        playerLi.className = "player";
        playerLi.onclick = function(){
            openDetail(JSON.stringify(playerList.outfielder[i]));
        };
...
function openDetail(detailData){
    localStorage.setItem('detailData', detailData);
    location.href = "playerDetail.html";
}
